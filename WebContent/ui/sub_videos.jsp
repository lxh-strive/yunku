		...
		<div class="middle">
			<div>
				<ul class="video_types">
					<c:choose>
						<c:when test="${not empty requestScope.videoSubType}">
							<li><b><a
									href="${applicationScope.basePath}videos/search_parent?parent_id=${requestScope.videoSubType.parentId}">${requestScope.videoSubType.parentType.name}(${requestScope.videoSubType.parentType.counts})</b></a>
							</li>
							<li>>>${requestScope.videoSubType.name}(${requestScope.videoSubType.counts})</li>
						</c:when>
					</c:choose>
				</ul>
			</div>
